...
HRESULT CEdit_Mesh::Initialize_Prototype(MODELTYPE eType, const SAVE_MESH& pMesh, const vector<class CEdit_Bone*>& Bones, const vector<SAVE_BONE>& BonesData)
{
	WideCharToMultiByte(CP_ACP, 0, pMesh.szName.c_str(), -1,
...
HRESULT CEdit_Mesh::Ready_Vertices_For_NonAnim(const SAVE_MESH& pMesh)
{
...
HRESULT CEdit_Mesh::Ready_Vertices_For_Anim(const SAVE_MESH& pMesh, const vector<CEdit_Bone*>& Bones, const vector<SAVE_BONE>& BonesData)
{
	m_iVertexStride = sizeof(VTXANIMMESH);
	...
	m_iNumBons = m_MeshData.iNumBones = pMesh.iNumBones;
...
CEdit_Mesh* CEdit_Mesh::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, MODELTYPE eType, const SAVE_MESH& pMesh, const vector<class CEdit_Bone*>& Bones, const vector<SAVE_BONE>& BonesData)
{
	CEdit_Mesh* pInstance = new CEdit_Mesh(pDevice, pContext);
